    ...
    private List<BusinessData> businessData(User user) {
        return businessProfileRepository.findByUser(user).stream()
                .map(business -> new BusinessData(
                        ...
                        business.getName(),
                        business.getCategory().name(),
                        business.getServiceTypes().stream().map(Enum::name).sorted().toList(),
                        business.getEventTypes().stream().map(Enum::name).sorted().toList(),
                        business.getOtherServiceDetails(),
                        business.getDescription(),
                        business.isNationwide() ? "Toată țara" : String.join(", ", business.getServiceCounties()),
                        business.getPhone(),
                        business.getEmail(),
                        business.isEmailVerified(),
                        business.getEmailVerifiedAt(),
                        business.getWebsite(),
